...
**Lifecycle (fresh load):** `Load(persistentState)` → [PhaseStarting] → `Begin()` → [PhasePlaying] → `Update(dt)` → game calls `gameOver()` → `End()` → results posted to chat as a system message → `Unload() any` → persistent state saved → [PhaseNone]
